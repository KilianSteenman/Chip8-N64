 ...
 ******************************************************************************/
int
sha1digest(uint8_t *digest, char *hexdigest, const uint8_t *data, int databytes)
{
#define SHA1ROTATELEFT(value, bits) (((value) << (bits)) | ((value) >> (32 - (bits))))
    ...
    if (hexdigest)
    {
        snprintf (hexdigest, 41, "%08x%08x%08x%08x%08x",
                  (unsigned int)H[0],(unsigned int)H[1],(unsigned int)H[2],(unsigned int)H[3],(unsigned int)H[4]);
    }
